...
- **Agent-image publishing is now a repo workflow instead of a manual push.** The dispatch-only `publish-agent-image` workflow builds the multi-arch agent image and pushes it to Docker Hub: an immutable `agent-<version>-<sha12>` candidate plus the moving `agent-alpha` alias. No change for installs: local builds and the hardened pin are untouched.
- **CVE gates on agent images.** The publish workflow refuses to push an image carrying critical vulnerabilities that have a fix available (grype), and `versions.json` repin PRs now scan the hardened image the same way before it can be pinned.
