...
- **Agent memory now gets scheduled maintenance.** A new agent is seeded at its first conversation with one paused task series — `memory hygiene` (Sundays 08:00) — driving a new `memory-hygiene` container skill that audits indexes, OKF frontmatter, links, contradictions and stale dates, then repairs what is safe and re-indexes. Seeded paused, so nothing runs until you enable it with `ncl tasks resume`; note that resume does not move the stored run time, so use `ncl tasks run` for an immediate confirming pass. A series you delete is never recreated, and existing agents are left alone (docs/memory.md has the command to add it). A pass may delete a fact only when it is clearly obsolete and already safely represented elsewhere. See [docs/memory.md](docs/memory.md#periodic-maintenance).
