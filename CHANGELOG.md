...
- **New skill: `/add-audit` — opt-in local audit log for the `ncl` surface.** Every command through the dispatcher (both transports, scope denials and approval holds included) becomes one append-only, SIEM-shaped NDJSON event under `data/audit/<UTC-day>.ndjson`; `details` keep only flag names plus a safe-enum allowlist, so raw argument values — where secrets hide — are never written. Read back with `ncl audit list` (filters + `--format ndjson`); in-process exporters plug in via `registerAuditHook`. Off unless `AUDIT_ENABLED=true` (`AUDIT_RETENTION_DAYS` default 90); ships as a skill whose only core footprint is the `withAudit(dispatch)` composition plus a resource-barrel import, guarded by a wiring test.
