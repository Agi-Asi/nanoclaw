...
- [BREAKING] **Chat SDK core and all `@chat-adapter/*` pins move 4.29.0 -> 4.32.0.** Fixes Telegram dropping any message whose link URL has an odd number of `_`, `*` or `~` (the OneCLI connect link): the adapter truncated the URL and Telegram rejected the message. Also in 4.32.0: Slack stops turning `@name` inside code spans and URLs into mentions; Discord renders bare URLs plainly; GitHub, Linear and Resend engage on an earlier @mention in a burst; WhatsApp Cloud typing becomes a live Graph call that also marks the message read. **Migration:** run `/update-nanoclaw` (or `/update-skills`), rebuild, restart. A hand-edited pin or `/add-<channel>` re-run leaves two `chat` cores and the build fails (`_subjectPromise`).
