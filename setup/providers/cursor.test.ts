...
describe('verifyCursorInstall', () => {
  it('passes on a tree with the cursor payload wired', () => {
...
describe('trunk reach-ins', () => {
  ...
  it('lists cursor in INSTALL_SKILLS so late-adopter provider-auth can install it', () => {
    const src = fs.readFileSync(path.join(process.cwd(), 'setup', 'provider-auth.ts'), 'utf-8');
    expect(src).toMatch(/cursor:\s*'\.claude\/skills\/add-cursor'/);
  });

  it('lists cursor in INSTALLABLE_PROVIDERS so the setup picker can offer it', () => {
    const src = fs.readFileSync(path.join(process.cwd(), 'setup', 'auto.ts'), 'utf-8');
    expect(src).toMatch(/value:\s*'cursor'/);
    expect(src).toContain('Installed ${agentProvider}, but its setup provider did not register.');
  });
});
